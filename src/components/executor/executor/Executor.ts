...
export class Executor<P> {
  ...
  public getSignedTransactions = async (
    ...
    const signedTransactions: Transaction[] | undefined= await this.signAllTransactions(
  ...
  public execute = async () => {
    const result = await this.fetch(this.params, this.connection, this.cluster)
      .catch((e) => {
        return { error: (e as Error).message ?? e, data: undefined };
      })
      .then((result) => result);
    const { data: transactions, error } = result;

    if (!transactions || transactions.length === 0) {
      this.onError(error ?? "Could not generate transactions");
      return;
    }
        ...
        if (signedTransactions) {
          ...
          while (signedTransactionsBatch.length > 0) {
            ...
            await promises
              ...
              .catch((err) => {
                console.log({ err });
                this.onError((err as Error).message ?? err);
              });
